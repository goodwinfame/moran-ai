...
| D4 | Agent 配置格式 | Markdown frontmatter（`.opencode/agents/*.md`） | §4.3 |
...
OpenCode 支持 Markdown frontmatter 格式的 Agent 配置文件，存放在 `.opencode/agents/` 目录：
...
```markdown
---
# .opencode/agents/jiangxin.md
description: "匠心 — 世界观、角色、结构设计专家。负责创建和完善世界设定、角色体系、故事大纲。"
model: anthropic/claude-sonnet-4-20250514
temperature: 0.4
tools:
  # 允许使用的 MCP 工具（白名单）
  moran-mcp_world_setting_create: true
  moran-mcp_world_setting_read: true
  moran-mcp_world_setting_update: true
  moran-mcp_world_subsystem_create: true
  moran-mcp_world_subsystem_update: true
  moran-mcp_world_consistency_check: true
  moran-mcp_location_create: true
  moran-mcp_location_update: true
  moran-mcp_glossary_create: true
  moran-mcp_glossary_update: true
  ...
  moran-mcp_arc_detail_create: true
