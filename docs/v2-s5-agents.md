...
- **角色**：五维心理模型（GHOST → WOUND → LIE → WANT ↔ NEED），关系网络，成长弧线
...
**关键能力**：
- 文风引擎：混合格式（YAML 约束 + 散文描述 + 示例段落），纯文风不绑定题材
- 题材技法加载：`context_assemble` 自动从知识库加载 `category='genre'` 条目
...
**双场景**：
- **本项目分析**（内部）：章节归档后自动触发，九维评分 + 趋势追踪
- **外部作品分析**（外部）：用户请求分析外部小说技法，产出分析报告 + 可操作经验（写入知识库"沉淀"分类）
...
| 知识库 | 博闻 | `knowledge_entries`, `lessons` (仅创作技法: 技巧/题材/风格/教训/沉淀) |
