...
## Phase 2: Agent 接入（10 个 Agent）+ Session 管理 (~5 weeks)
...
- **执笔 (Writer)**: 负责章节起草与局部修订。
- **明镜 (Reviewer)**: 负责四轮审校与质量把关。
- **载史 (Archivist)**: 负责章节归档、摘要与状态快照。
- **博闻 (Knowledge Keeper)**: 负责知识库与一致性辅助查询。
- **析典 (Analyst)**: 负责九维分析与作品诊断。
- **书虫 (Reader)**: 负责读者视角反馈。
- **点睛 (Copywriter)**: 负责标题、简介与宣传语。
...
| **信息面板 - 审校 Tab** | 五维雷达图动画展示单章质量。详细列出每轮审校的分数变化、具体的修改意见标注与采纳情况。 | 前端 | 待办 |
...
- [ ] 与墨衡的全流程对话体验丝滑，支持 Markdown 解析、代码块预览；关系可视化不使用 Mermaid，技术选型待定。
