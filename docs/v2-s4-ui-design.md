...
V2 延续 V1 的设计理念：执笔（zhibi）是唯一的"写手角色"，但底层可切换不同的**文风配置**，每个文风拥有一个**人格子名**和可选的**显示别名**，赋予用户"不同写手轮流执笔"的创作体验。
...
| 子名 | 显示别名 | 文风特征 | 推荐模型 | 模型选择理据 |
|------|----------|----------|----------|-------------|
| 云墨 | — | 均衡万用、自然流畅 | Claude Sonnet | 均衡全面，性价比高 |
| 剑心 | 寒刃白描 | 冷峻简约、短句、白描、动作化叙事 | Kimi K2 | 中文古风语感最佳，长篇节奏稳定 |
| 星河 | — | 精确、技术感、理性叙述 | GPT-4o | 逻辑严密，技术描写精准 |
| 素手 | 暖阳絮语 | 温暖细腻、长句、情感细写、氛围渲染 | Claude Opus | 情感表达细腻，心理描写出色 |
| 烟火 | — | 市井烟火气、口语化、快节奏 | GPT-4o | 现代感强，对话自然 |
| 暗棋 | 迷雾潜行 | 层层递进、信息控制、悬念留白 | Claude Opus | 长链条推理能力强 |
| 青史 | 苍穹长歌 | 典雅庄重、文白混用、时代语感 | Claude Opus | 古文功底好，时代感把控准 |
| 夜阑 | 暗夜低吟 | 压抑、感官描写密集、心理暗示 | Gemma4 | 氛围渲染直觉强 |
| 谐星 | 嬉笑怒骂 | 轻快、节奏明快、反差幽默 | GPT-4o | 幽默感和节奏把控好 |

> **显示规则**：有别名的显示为"执笔·显示别名"（如"执笔·寒刃白描"），无别名的显示为"执笔·子名"（如"执笔·云墨"）。人格子名始终作为内部 ID 使用。
...
**用户自定义文风**：用户可创建新文风并自取子名和别名（如子名"朝歌"，别名"晨曦高歌"），选择推荐模型，配置文风描述。

### 9.2 交互呈现

**Agent 状态条**（输入框上方）：
```
🟢 执笔·寒刃白描 写作第 38 章 · 1,847字
```
...
|---------|---------|--------------|---------|
| 仙侠 | 剑心（寒刃白描） | 修炼体系写法、境界突破节奏、战斗描写技巧 | 冷峻文风 + 仙侠技法 |
| 仙侠 | 素手（暖阳絮语） | 同上 | 温暖文风 + 仙侠技法 = 温情仙侠 |
| 悬疑 | 暗棋（迷雾潜行） | 线索埋设、信息不对称、推理链条 | 悬念文风 + 悬疑技法 |
