...
知识库按分类展示**结构化知识条目**，是全书的"记忆中枢"。随着创作推进，博闻自动从章节归档、审校报告、设定变更中提取和维护知识。
...
| 条目内容 | 只读 | 修改需在聊天中说"更新源质的设定" |

#### 空状态

```
知识库暂无条目。随着创作推进，博闻会自动收集和整理知识。
```
...
V2 延续 V1 的设计理念：执笔（zhibi）是唯一的"写手角色"，但底层可切换不同的**风格配置**，每个风格拥有一个**诗意子名**，赋予用户"不同写手轮流执笔"的创作体验。

### 9.1 风格子名体系

子写手名跟的是**风格**，不是模型。每个风格配置关联一个推荐模型（该风格效果最佳的模型），用户感知到的是风格人格，不关心底层跑的哪个 LLM。

**预设风格**：

| 子名 | 显示名 | 题材 | 核心特征 | 推荐模型 | 模型选择理据 |
|------|--------|------|----------|----------|-------------|
| 云墨 | 执笔·云墨 | 默认/万用 | 均衡文风、自然流畅 | Claude Sonnet | 均衡全面，性价比高 |
| 剑心 | 执笔·剑心 | 仙侠/武侠 | 古风语感、修炼体系、战斗场景 | Kimi K2 | 中文古风语感最佳，长篇节奏稳定 |
| 星河 | 执笔·星河 | 硬核/太空 | 硬核设定、科技描写、哲学思辨 | GPT-4o | 逻辑严密，技术描写精准 |
| 素手 | 执笔·素手 | 情感/关系 | 情感细腻、关系推拉、氛围渲染 | Claude Opus | 情感表达细腻，心理描写出色 |
| 烟火 | 执笔·烟火 | 现代/都市 | 现代语感、快节奏、职场/社会 | GPT-4o | 现代感强，对话自然 |
| 暗棋 | 执笔·暗棋 | 推理/悬疑 | 伏笔密集、信息不对称、推理链条 | Claude Opus | 长链条推理能力强 |
| 青史 | 执笔·青史 | 朝堂/历史 | 时代语感、历史细节、朝堂博弈 | Claude Opus | 古文功底好，时代感把控准 |
| 夜阑 | 执笔·夜阑 | 惊悚/恐怖 | 感官压迫、心理暗示、节奏控制 | Gemma4 | 氛围渲染直觉强 |
| 谐星 | 执笔·谐星 | 轻松/喜剧 | 节奏明快、反差幽默、角色互动 | GPT-4o | 幽默感和节奏把控好 |
...
genre: 仙侠/武侠
model:
  recommended: claude-opus-4     # 该风格效果最佳的模型
...
**用户自定义风格**：用户可创建新风格并自取子名（如"执笔·朝歌"），选择推荐模型，配置文风描述。
...
- **默认**：墨衡根据项目创建时的题材自动匹配预设风格（赛博修仙 → 剑心）
- **用户指定**：在对话中指定（"用星河写这一章"、"换暗棋风格"）
- **墨衡推荐**：根据章节类型推荐风格切换，以 Question Panel 方式呈现
...
风格绑定的推荐模型是默认选择，用户可在项目设置中覆盖：

| 层级 | 说明 |
|------|------|
| 风格默认 | 每个风格配置中的 `model.recommended`，开箱即用 |
| 项目级覆盖 | 项目设置中为某个风格指定不同模型（如剑心改用 GPT-4o 省成本） |
| 全局偏好 | 用户级设置，所有项目统一偏好（如"所有风格都用 Claude"） |
